var graph = graphLib('#graph', 
   {
      ...
      style: {
        person: { r: 10},
        square: { tag: 'rect', attributes: { width: 10, height: 10}}
      } 
   });
...
QUnit.test('different svg tags by type', function(assert){
   var node = {label: 'hi', type: 'square', id: 0};
   graph.addNodes([node]);
   assert.equal($('.node rect').length, 1);
   graph.clear();
});
