 ...
 *
 * A simple graph library for easily visualizing a graphs
...
function graphLib(selector, conf)
{
   ...
   function getNodes(selector)
   {
      ...
      if (selector !== undefined && selector !== null && typeof selector === 'object'){
         ...
         ret = [];
         data.nodes.forEach(function(n,k){
           if (isSubset(n, selector))
           {
              ret.push(n);
           }
         });
   ...
   function getLinks(selector)
   {
      ...
      if (selector !== undefined && selector !== null && typeof selector === 'object')
      {
         ...
         data.links.forEach(function(l,k){
            if (isSubset(l.target,selector) || isSubset(l.source,selector))
            {
               ret.push(l);
            }
         });
   ...
   /**
    * adds nodes in the minimum form {label: <string or string[] label for the node>, id: <unique id for the node>}
   ...
   function updateNodes()
   {
     ...
     var element = nodeEnter.append(function(d){
        return (config.style[d.type] && config.style[d.type].tag) || config.style.defaultStyle.tag;
     });

     element.each(function(d){
        var style = (config.style[d.type] && config.style[d.type].attributes) || config.style.defaultStyle.attributes;
